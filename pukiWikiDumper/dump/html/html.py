...
def dump_HTML(puki_url, dumpDir,
...
def dump_html_page(dumpDir, index_of_title, title, puki_url, session: requests.Session, current_only: bool = False):
    r = session.get(puki_url, params={'do': running_config.export_xhtml_action, 'id': title})
    ...
    revs = get_revisions(puki_url=puki_url, session=session, title=title, msg_header=msg_header)

    for rev in revs[1:]:
        if 'id' in rev and rev['id']:
            try:
                r = session.get(puki_url, params={'do': running_config.export_xhtml_action, 'id': title, 'rev': rev['id']})
                r.raise_for_status()
                if r.text is None or r.text == '':
                    raise Exception(f'Empty response (r.text)')
